...
export async function answerKpssQuestion(message: string, options: TutorOptions = {}): Promise<KpssTutorAnswer> {
  ...
  if (!clean) {
    const reply = "Ben sadece KPSS Coğrafya konu/kavram tanımları ve 'nedir' tarzı bilgi sorularına cevap verebilirim. Lütfen öğrenmek istediğiniz kavramı (örn: 'yasama dokunulmazlığı nedir' veya 'anayasa mahkemesi') yazın.";
  ...
  if (greeting) {
    const reply = "Merhaba! Ben KPSS Coğrafya Rehberi'yim. **[UYARI: Ben sadece konu/kavram tanımları ve 'nedir' tarzı bilgi sorularına cevap verebilirim.]** Sormak istediğiniz KPSS Coğrafya kavramını (örn: 'idari vesayet nedir?') yazın, veritabanından tarayıp açıklayayım.";
  ...
  if (!isDefinitionQuery) {
    const reply = "Bu chatbot sadece KPSS Coğrafya konuları ve 'nedir' tarzı bilgi sorularına yanıt verecek şekilde sınırlandırılmıştır. Lütfen doğrudan öğrenmek istediğiniz anayasal/hukuki kavramı (örn: 'hükümsüzlük nedir?') sorunuz.";
  ...
  if (localSearch) {
    ...
    ans.reply = `[Kaynak Konu: ${ans.matchedTitle || "Coğrafya Bilgisi"}]\n\n${ans.reply}`;
    return ans;
  }
  
  const reply = "Aradığınız kavramı veritabanımızda bulamadım. Bu rehber sadece KPSS Coğrafya konu/kavram tanımları ve 'nedir' tarzı bilgi sorularına cevap verebilir. Lütfen aramanızı daha net bir kavramla (örn: 'idari vesayet') tekrar deneyin.";
