...
if (totalQuestionsCount > 0) {
  ...
  for (let i = 1; i <= 100; i++) {
    ...
    let title = \`KPSS Coğrafya Deneme Sınavı - \${i}\`;
    if (i === 100) title = "KPSS Coğrafya 100. Yıl Özel Denemesi";
    else if (i % 10 === 0) title = \`KPSS Coğrafya Altın Deneme - \${i / 10}\`;
  ...
  examConfigs.forEach((config) => {
    ...
    exams.push({
      id: config.id,
      title: config.title,
      description: \`ÖSYM formatına ve güncel KPSS Coğrafya müfredatına uygun olarak özenle hazırlanmış \${config.count} soruluk \${config.difficulty} seviye deneme sınavı.\`,
...
export const recommendations: StudyRecommendation[] = [
  {
    ...
    title: "Bugünün KPSS Coğrafya çalışma akışı",
...
// Explicit mapping for encyclopedia entry IDs to Cografya topic IDs
